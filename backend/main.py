...
RawCitation = Dict[str, Optional[str]]


def _normalise_raw_entry(entry: Any) -> Optional[RawCitation]:
    """Convert a JSON reference entry into a serialisable mapping."""

    if isinstance(entry, str):
        title = entry.strip()
        if not title:
            return None
        return {"title": title, "pmid": None, "doi": None}
...
def load_references() -> Dict[str, List[RawCitation]]:
    ...
    references: Dict[str, List[RawCitation]] = {}
    for raw_name, entries in refs_data.items():
        ...
        parsed_entries: List[RawCitation] = []
        for entry in raw_list:
            normalised = _normalise_raw_entry(entry)
            if normalised:
                parsed_entries.append(normalised)
        if parsed_entries:
            references.setdefault(canon_name, []).extend(parsed_entries)
    return references


REFERENCES: Dict[str, List[RawCitation]] = load_references()
...
def build_citation_payload(receptor_names: Iterable[str]) -> Dict[str, List[Citation]]:
    ...
    for name in receptor_names:
        ...
        if not refs:
            continue
        payload[canon] = [Citation.parse_obj(ref) for ref in refs]
    return payload
