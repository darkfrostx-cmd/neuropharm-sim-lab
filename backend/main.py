...
def clone_citation(ref: Citation) -> Citation:
    """Return a fresh ``Citation`` instance compatible with Pydantic v1/v2."""

    if hasattr(ref, "model_dump"):
        data = ref.model_dump()
    else:  # pragma: no cover - exercised when running under Pydantic v1
        data = ref.dict()
...
def build_citation_payload(receptor_names: Iterable[str]) -> Dict[str, List[Citation]]:
    ...
    for name in receptor_names:
        ...
        if not refs:
            continue
        payload[canon] = [clone_citation(ref) for ref in refs]
    return payload
